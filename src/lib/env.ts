...
export const env = {
  ...
  get SUPABASE_ANON_KEY() {
    ...
    // accept either env var name for backward compatibility. Use `||` (not
    // `??`) so an empty string left in `.env` after migration falls through.
    return required(
  ...
  get SUPABASE_SERVICE_ROLE_KEY() {
    ...
    // in late 2024. We accept either env var name. Use `||` (not `??`) so an
    // empty string left in `.env` after migration falls through.
    return required(
